import { HttpActuatorFetcher } from './http-actuator-fetcher';

jest.spyOn(global, 'fetch');

const fetchMock: jest.Mock = fetch as jest.Mock;

describe('HttpActuatorFetcher', () => {
  beforeEach(() => {
    fetchMock.mockClear();
  });
  ...
  it('Should resolve request result', async () => {
    ...
    const result = await new HttpActuatorFetcher('url', 'method', { key: 'value' }, 'body', 1000).request();
  ...
  it('Should reject request result', async () => {
    ...
    await expect(new HttpActuatorFetcher('', '', undefined, '').request()).rejects.toBeDefined();
  });
});
