...
export class StreamSensor extends Sensor {
  private server: any;
  private stream: any;
  ...
  constructor(sensorAttributes: SensorModel) {
    ...
    if (this.response && typeof sensorAttributes.response != 'string') {
  ...
  private reuseServer(): Promise<void> {
    return new Promise((resolve, reject) => {
      try {
        ...
        Logger.error(`Stream sensor errored: ` + err);
        this.createServer()
          .then(() => resolve())
  ...
  private persistStream() {
    if (this.stream) {
      if (this.saveStream) {
        Logger.debug(`Persisting sensor ${this.type} stream ${this.saveStream}`);
...
export function entryPoint(mainInstance: MainInstance): void {
  const createFunction = (sensorModel: SensorModel) => new StreamSensor(sensorModel);
  ...
  const tcp = new SensorProtocol('tcp', createFunction, docs);

  const uds = new SensorProtocol('uds', createFunction, docs);

  const ssl = new SensorProtocol('ssl', createFunction, docs);
