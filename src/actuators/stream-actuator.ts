...
class StreamActuator extends Actuator {
  ...
  private sendReusingStream(resolve: any, reject: any) {
    ...
    if (!this.loadedStream) {
      ...
      this.sendData(this.loadedStream, resolve, reject);
    }
  }
  ...
  private sendCreatingStream(resolve: any, reject: any) {
    ...
    this.createStream()
      .then((stream: any) => {
        ...
        this.sendData(stream, resolve, reject);
      })
      .catch(err => {
        reject(err);
      });
  ...
  private sendData(stream: any, resolve: any, reject: any) {
    Logger.debug(`${this.type} client sending message`);
    ...
    stream.write(stringifyPayload, () => {
      Logger.debug(`${this.type} client sent message`);
...
export function entryPoint(mainInstance: MainInstance): void {
  const createFunction = (actuatorModel: ActuatorModel) => new StreamActuator(actuatorModel);
  const docs: ProtocolDocumentation = {
    description: 'The stream sensor provides implementations of TCP/UDS servers',
  ...
  const tcp = new ActuatorProtocol('tcp', createFunction, docs);
  const uds = new ActuatorProtocol('uds', createFunction, docs);
  const ssl = new ActuatorProtocol('ssl', createFunction, docs);
